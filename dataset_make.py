...
def generate_and_save(generator, num_classes, samples_per_class, num_copies, prefix):
    ...
    save_npz(f"{prefix}_s{num_tiles}_c{num_copies//2}.npz", xyac[:half_total_samples], labels[:half_total_samples])
    save_npz(f"{prefix}_s{num_tiles}_c{num_copies}.npz", xyac, labels)
...
if __name__ == "__main__":
    ...
    generate_and_save(gen6, NUM_CLASSES, SAMPLES_PER_CLASS, num_random_copies, prefix="datasets/hex")
