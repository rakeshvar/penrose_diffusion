...
class PermutationBaseScipy(torch.nn.Module, ABC):
    """Abstract base class for scipy-based permutation matching."""
    
    def __init__(self):
        super().__init__()

    def _compute_assignment(self, truth, preds, colors):
        """
...
class PermutationLossScipy(PermutationBaseScipy):
    """Computes permutation loss using scipy's linear_sum_assignment."""
    
    def forward(self, truth, preds, colors):
        cost_matrix, b_flat, t_flat, p_flat = self._compute_assignment(truth, preds, colors)
        # Advanced indexing into the gradient-tracking tensor
        return cost_matrix[b_flat, p_flat, t_flat].sum() / truth.numel()


class PermutationOnlyScipy(PermutationBaseScipy):
    """Returns only the assignment indices without computing loss."""
    
    def forward(self, truth, preds, colors):
        _, b_flat, t_flat, p_flat = self._compute_assignment(truth, preds, colors)
